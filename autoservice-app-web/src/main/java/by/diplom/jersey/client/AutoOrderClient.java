package by.diplom.jersey.client;
...
import by.diplom.endpoints.AutoOrder;
import by.diplom.jersey.config.AutoOrderRestConfig;
...
public class AutoOrderClient {
    ...
    public AutoOrder createNew(AutoOrder newOrder) {
        ...
        try {
            ...
            WebTarget add = client.target(AutoOrderRestConfig.AUTOORDER_SERVICE_PATH).path(
                AutoOrderRestConfig.CREATE_NEW_PATH);
    ...
    public AutoOrder modify(AutoOrder order, AutoOrder withOrder) {
        ...
        try {
            ...
            WebTarget autoOrderById = client
                .target(AutoOrderRestConfig.AUTOORDER_SERVICE_PATH)
                .path(AutoOrderRestConfig.ORDER_ID_PATH)
                .resolveTemplate(AutoOrderRestConfig.ORDER_ID_TEMPLATE, order.getOrderId());
    ...
    public List<AutoOrder> findAll() {
        ...
        try {
            client = ClientBuilder.newClient();
            WebTarget target = client.target(AutoOrderRestConfig.AUTOORDER_SERVICE_PATH);
    ...
    public AutoOrder findById(String id) {
        ...
        try {
            ...
            WebTarget autoOrderById = client.target(
                AutoOrderRestConfig.AUTOORDER_SERVICE_PATH).path(AutoOrderRestConfig.ORDER_ID_PATH).resolveTemplate(
                AutoOrderRestConfig.ORDER_ID_TEMPLATE, id);
    ...
    public boolean delete(AutoOrder order) {
        ...
        if (response != null && response.getStatus() == HttpStatus.NO_CONTENT.value()) {
